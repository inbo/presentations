...
- look for exterme values given the model
    - strong residuals
...
- measure the shortest distance $r_{ij}$ from sampling point $i$ to nearest plant part of species $j$
    ...
    - $r_{ij} = r_{max}$ when species $j$ is not found with $r_{max}$ around the sampling point
- requires several sampling points per location
- measured continous data instead of estimated ordinal data
- $r^2 / r^2_{max} \sim BI(p_0, p_1, \mu, \phi)$ 
    - $p_0$: cover estimated by point sampling
    - $1 - p_1$: presence with a circle of $r_{max}$
- integral occurence probability
    - $1 - \mbox{E}(y) = 1 - (p_1 + (1 - p_0 - p_1) \mu)$
