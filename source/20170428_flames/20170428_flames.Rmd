...
ggplot(shapes, aes(x = x, y = y)) +
  geom_path(aes(group = phase, colour = phase %in% c(0, 5))) +
  geom_segment(
    data = triple_arrows, 
    aes(xend = xend, yend = yend), 
    arrow = arrow(length = unit(2, "mm"), type = "closed")
  ) +
  geom_text(data = texts, aes(label = gsub("Phase V\n", "", label), colour = phase %in% c(0, 5))) +
  scale_colour_manual(values = c("TRUE" = inbo.steun.blauw, "FALSE" = NA), guide = "none") +
  coord_fixed() + 
  theme_blank()
```
